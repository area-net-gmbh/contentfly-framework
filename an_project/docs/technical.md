...
| ~~A-5~~ | ~~`referrer`-Tokens laufen nie ab, und der Token-String kommt beim Anlegen vom Client (`Controller/SystemController.php:159`)~~ **Aufgelöst mit `000-000-0030`:** Ohne `token` erzeugt `addToken` den Wert selbst; ein mitgeschickter braucht mindestens 32 Zeichen mit 10 verschiedenen, sonst `400`. Ablauffrist und Bremse beim Vorzeigen sind geprüft und begründet verworfen, die Begründung steht an `TokenHandler::timeoutApplies()` und `fromDatabase()`: Ein API-Token steckt in einem Fremdsystem, der Widerruf läuft über `deleteToken`, und ein erzwungen starker Token ist online nicht durchprobierbar. Von Hand gewählte Bestandstokens bleiben gültig, siehe `breaking-changes.md`. | ~~Ratbare Dauerschlüssel möglich~~ |
...
| ~~A-7~~ | ~~Ein Upload behält die Endung des Clients; `data/files/` liegt im Document-Root. Eine hochgeladene `probe-upload.php` wurde beim Abruf ausgeführt (`EXECUTED-42`). Gefunden am Bestandsprojekt UFP (`007-005-0001`), nicht im Review vom 2026-09-04.~~ **Behoben mit `000-000-0038`:** `Classes/File/UploadValidator` weist ausführbare Endungen in jedem Namensteil und Server-Konfigurationsnamen mit `415` ab, bevor etwas geschrieben wird; der gespeicherte Name wird vom Framework gebildet. Optional prüft `FILE_ALLOWED_TYPES` Endung und Inhaltstyp. | ~~Codeausführung mit jedem gültigen Token~~ |
| ~~A-8~~ | ~~CORS spiegelt den `Origin` jeder Anfrage mit `Access-Control-Allow-Credentials: true`; `APP_ALLOW_ORIGIN` wird nie gelesen. Gefunden am Bestandsprojekt UFP (`007-005-0001`), nicht im Review vom 2026-09-04.~~ **Behoben mit `000-000-0039`:** `Classes/Security/CorsPolicy` nennt nur eine in `APP_ALLOW_ORIGIN` eingetragene Herkunft, genau diese und mit Credentials; ohne Eintrag keine. `*` ist möglich, dann ohne Credentials. Jede Antwort trägt `Vary: Origin`. | ~~Fremde Seiten lesen Antworten mit den Credentials eines angemeldeten Benutzers~~ |
