...
namespace RectorPrefix202402\React\Dns\Query;

use RectorPrefix202402\React\Dns\Model\Message;
use RectorPrefix202402\React\Dns\Protocol\BinaryDumper;
use RectorPrefix202402\React\Dns\Protocol\Parser;
use RectorPrefix202402\React\EventLoop\Loop;
use RectorPrefix202402\React\EventLoop\LoopInterface;
use RectorPrefix202402\React\Promise\Deferred;
/**
...
class TcpTransportExecutor implements ExecutorInterface
{
    ...
    public function query(Query $query)
    {
        ...
        if ($length > 0xffff) {
            return \RectorPrefix202402\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Query too large for TCP transport'));
        }
        ...
        if ($this->socket === null) {
            ...
            if ($socket === \false) {
                return \RectorPrefix202402\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Unable to connect to DNS server ' . $this->nameserver . ' (' . $errstr . ')', $errno));
            }
     ...
     * @param int    $code
     */
    public function closeError($reason, $code = 0): void
    {
        $this->readBuffer = '';
        if ($this->readPending) {
